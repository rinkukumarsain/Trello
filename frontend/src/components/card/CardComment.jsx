...
const CardComments = ({ card, comments, setComments }) => {
  const [comment, setComment] = useState("");
  const [commentLoading, setCommentLoading] = useState(false);
  const [showDetails, setShowDetails] = useState(true); // for toggle
          ...
          <div className="mt-6 space-y-4 max-h-[300px] overflow-y-auto pr-1">
            {comments.length === 0 ? (
              ...
              comments.map((c, index) => (
                <div
                  key={index}
                  className="bg-gray-700 p-3 rounded-md text-sm text-white"
                >
